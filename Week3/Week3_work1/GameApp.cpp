...
void GameApp::UpdateScene(float dt)
{
    ...
    XMMATRIX R = XMMatrixRotationZ(phi);
    ...
    //m_PSConstantBuffer.rotMat = (A);
    m_PSConstantBuffer.rotMat = XMMatrixTranspose(R);
    ...
    if (ImGui::Begin("Lighting"))
    {
        ...
        else if (curr_light_item == 1)
        {
            ...
            float x = m_PSConstantBuffer.pointLight.position.x;
            float y = m_PSConstantBuffer.pointLight.position.y;
            if (ImGui::SliderFloat("LightPosX", &x, -10.0f, 10.0f))
            {
                m_PSConstantBuffer.pointLight.position.x = x;
            }
            if (ImGui::SliderFloat("LightPosY", &y, -10.0f, 10.0f))
            {
                m_PSConstantBuffer.pointLight.position.y = y;
            }
        }
        else
        {
            ...
            float x = m_PSConstantBuffer.spotLight.position.x;
            float y = m_PSConstantBuffer.spotLight.position.y;
            if (ImGui::SliderFloat("LightPosX", &x, -10.0f, 10.0f))
            {
                m_PSConstantBuffer.spotLight.position.x = x;
            }
            if (ImGui::SliderFloat("LightPosY", &y, -10.0f, 10.0f))
            {
                m_PSConstantBuffer.spotLight.position.y = y;
            }
        }
        ...
        float roughness = m_PSConstantBuffer.material.specular.w;
        if (ImGui::SliderFloat("Roughness", &roughness, 0.01f, 1.0f))
        {
            m_PSConstantBuffer.material.specular.w = roughness;
        }
    }
    ImGui::End();
    ImGui::Render();
...
void GameApp::DrawScene()
{
    ...
    m_pd3dImmediateContext->OMSetDepthStencilState(pDSState.Get(), 0);
...
bool GameApp::InitEffect()
{
    ...
    // 创建顶点着色器
    HR(CreateShaderFromFile(L"HLSL\\work1_TEX_VS.cso", L"HLSL\\work1_TEX_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
    ...
    // 创建像素着色器
    HR(CreateShaderFromFile(L"HLSL\\work1_TEX_PS.cso", L"HLSL\\work1_TEX_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
...
bool GameApp::InitResource()
{
    ...
    HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\WoodCrate.dds", nullptr, m_pTextureSRV.GetAddressOf()));
    ...
    // 点光
    m_PointLight.position = XMFLOAT3(10.0f, 10.0f, -10.0f);
    ...
    m_PSConstantBuffer.material.specular = XMFLOAT4(0.5f, 0.5f, 0.5f, 0.2f);
    // 使用默认平行光
   ...
   /* rasterizerDesc.FillMode = D3D11_FILL_WIREFRAME;
	...
	// Alpha = SrcAlpha
	blendDesc.AlphaToCoverageEnable = false; // 默认关闭，这里
	blendDesc.IndependentBlendEnable = false; // 是否每个渲染目标都有独立的混合混合描述，关闭的话都使用索引为0的描述信息
	rtDesc.BlendEnable = true; // 是否开启混合
	rtDesc.SrcBlend = D3D11_BLEND_SRC_ALPHA; // 源颜色混合因子
	rtDesc.DestBlend = D3D11_BLEND_INV_SRC_ALPHA; // 目标颜色混合因子
	rtDesc.BlendOp = D3D11_BLEND_OP_ADD; // 颜色混合运算符
	rtDesc.SrcBlendAlpha = D3D11_BLEND_ONE; // 源Alpha混合因子
	rtDesc.DestBlendAlpha = D3D11_BLEND_ZERO; // 目标Alpha混合因子
	rtDesc.BlendOpAlpha = D3D11_BLEND_OP_ADD; // Alpha混合运算符
	rtDesc.RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL; // 这行是什么意思，这一行就是渲染出来的关键：可以写入所有颜色
    ...
    HR(m_pd3dDevice->CreateDepthStencilState(&depthDesc, m_pDepthState.GetAddressOf()));

    // 绑定
    m_pd3dImmediateContext->OMSetDepthStencilState(m_pDepthState.Get(), 0); // 第二个参数是 StencilRef
    ...
    m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTextureSRV.GetAddressOf());
